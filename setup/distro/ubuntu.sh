...
PACKAGES=(
	lsb-release curl  gpg  nano  git sudo rsync openssh-client
...
_install_packages () {
  ...
  do
    ...
    _continue
    sudo apt-get install -y $i 1> /dev/null 2> .setup.log || _log_error "Could not isntall $1"
  done
}
...
_get_docker () {
  if ! _has_command docker; then
    ...
    sudo apt-get install -y docker-ce docker-ce-cli containerd.io  1> /dev/null 2> .setup.log || _log_error "Could not isntall $1"
  fi
}
