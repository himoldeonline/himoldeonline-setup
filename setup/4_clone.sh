...
cd $EDX_PLATFORM_ROOT git fetch --all --tags  1> /dev/null 2>> $_LOG_FILE && _info_ok || _log_tail_exit
cd $EDX_PLATFORM_ROOT && git checkout $TAG_EDX_PLATFORM  1> /dev/null 2>> $_LOG_FILE && _info_ok || _log_tail_exit
...
cd $OPENEDX_DEV_ROOT && git checkout $BRANCH_OPENEDX_DEV 1> /dev/null 2>> $_LOG_FILE && _info_ok || _log_tail_exit
